...
HAVING		COUNT(examquestion.ExamID) >= 	ALL
...
SELECT 		question.QuestionID, question.Content, COUNT(question.QuestionID) AS anwser_count
FROM 		question
JOIN		answer ON question.QuestionID = answer.QuestionID
GROUP BY	question.QuestionID
HAVING		COUNT(answer.AnswerID) >=	ALL
										(SELECT		COUNT(answer.AnswerID)
										FROM		answer
...
SELECT		`position`.PositionName, COUNT(`account`.AccountID) AS position_count
FROM		`position`, `account`
WHERE		`position`.PositionID = `account`.PositionID
GROUP BY	`position`.PositionID
HAVING		COUNT(`account`.AccountID) <= 	ALL
											(SELECT		COUNT(`account`.AccountID) AS position_count
                                            FROM		`position`, `account`
                                            WHERE		`position`.PositionID = `account`.PositionID
                                            GROUP BY	`position`.PositionID)
;
...
ORDER BY	question.QuestionID #list in ASC order too look more professional
;
/*==========================================================================================================*/
/*==========================================================================================================*/
/* Exercise 2: Union */
...
UNION 		#UNION will automatically remove duplicate
